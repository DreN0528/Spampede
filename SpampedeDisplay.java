...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		// Draw the board
		int x_space = (this.width - (Preferences.NUM_CELLS_WIDE * Preferences.CELL_SIZE)) / 2;
		int y_space = 60;
		
		for (int y = 0 ; y < Preferences.NUM_CELLS_TALL; y++) {
			for (int x = 0; x < Preferences.NUM_CELLS_WIDE ; x++) {
				this.drawSquare( x * Preferences.CELL_SIZE + x_space, y * Preferences.CELL_SIZE + y_space, this.theData.getCellColor(y , x ));
